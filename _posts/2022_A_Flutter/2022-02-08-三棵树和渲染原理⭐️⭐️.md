...
- [是如何更新的](#content3) 
- [一个典型示例](#content4) 
- [结合三棵树来说说CPU和GPU是如何工作的](#content5) 
- [iOS原生渲染和Flutter渲染有什么不同⭐️](#content6) 
...
2、 如果是StatefullWidget，element对象会持有state对象并管理它(创建、初始化、构建、销毁)。state持有widget对象。<br>
...
```
<span style="color:red;font-weight:bold;">注意：title是widget的属性。color是state的属性</span>
...
```
根据canUpdate返回值     
返回true<span style="color:red;">更新element持有的widget</span><br>
返回false<span style="color:red;">销毁当前element,创建新的element</span>
...
<span style="color:red;">所以title发生了变换变为了widgetb，而颜色没有发生变化还是statea持有的颜色。</span>


## <a id="content5">结合三棵树来说说CPU和GPU是如何工作的</a>
...
## <a id="content6">iOS原生渲染和Flutter渲染有什么不同⭐️</a>
