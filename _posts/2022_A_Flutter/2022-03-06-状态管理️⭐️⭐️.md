...
**为什么只能在 build 中使用？**     
`context.watch` 的核心是调用 `dependOnInheritedWidgetOfExactType`，向 `InheritedWidget` **注册当前 Element 的依赖关系**。当数据变化时，框架会通过这个依赖关系找到所有注册过的 Element，调用其 `markNeedsBuild()` 触发重新构建。     
而 `Element.markNeedsBuild()` 的设计目标就是**在 build 阶段标记脏节点，等待下一帧统一重建**。这套机制只有在 build 过程中才能正确建立「依赖 → 通知 → 重建」的闭环。     

具体原因有以下几点：     
① **依赖注册时机问题**：`watch` 内部会调用 `context.dependOnInheritedWidgetOfExactType`，它要求 `context` 处于「活跃可依赖」状态。在 `initState` 等生命周期中，Element 尚未完成 build，此时注册依赖会导致依赖关系混乱（可能注册到错误的 InheritedWidget，或注册后立即失效）。     
② **无法建立稳定的重建链路**：在回调（如 onPressed）中调用 `watch`，虽然不一定会立刻报错，但此时注册的依赖是「一次性」的——下一次数据变化时框架虽然会标记当前 Element 为 dirty，但由于不在 build 流程中，这种依赖注册不会被正确维护，容易出现「监听失效」或「内存泄漏」。     
③ **框架主动拦截**：Flutter 在 debug 模式下会检测 `dependOnInheritedWidgetOfExactType` 是否在错误的时机被调用，例如 `initState` 中调用会直接抛出断言错误：`"dependOnInheritedWidgetOfExactType() or dependOnAncestorWidgetOfExactType() cannot be called during initState"`。     

**源码佐证**：`context.watch` 实际上等价于：     
```dart
T watch<T>() {
...
```
而 `Provider.of(listen: true)` 最终会走到 `context.dependOnInheritedWidgetOfExactType(_inheritedProviderType)`，这一步就是「注册依赖」的关键。在 build 之外调用它，要么直接断言失败，要么导致依赖无法被正确清理和触发。     
...
**记忆口诀**：watch 看数据刷新 UI（build 内），read 拿对象调方法（build 外）。
...
**Demo 说明**：     
- 点击 `FloatingActionButton` 时，通过 `context.read` 拿到 `CounterNotifier` 调用 `increment()`。     
- `increment()` 内部修改 `count` 后调用 `notifyListeners()`。     
- `CounterText` 中的 `context.watch` 监听到了变化，触发自身 rebuild，显示最新数字。     
- 整个过程只有 `CounterText` 重新构建，`MyApp` 和 `Scaffold` 不会重建，体现了 Provider 的细粒度刷新能力。     
