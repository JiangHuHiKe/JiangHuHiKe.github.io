...
<span style="color:red;font-weight:bold;">1、Flutter 是 自己(Skia)在 GPU 上开的一块画布，它独立于系统合成流程，而不是系统合成流程的一部分。</span>  

<span style="color:red;font-weight:bold;">2、UIViewController的UIView才是APP要展示的画布，它是UIKit通过我们上边提到的一系列的流程最终显示到屏幕上的。</span>  

<span style="color:red;font-weight:bold;">3、Flutter这块画布自己不知道展示在哪里，要想展示出来，就要借助原生的这块画布，所以FlutterViewController的FlutterView是Flutter这块画布的容器。</span>       
  
理解了上边几条，我们也就理解了为什么在iOS和Flutter的混合工程中，FlutterViewController在创建的时候会跟FlutterEngine绑定，因为FlutterEngine中skia绘制的画布(离屏缓冲区)最终是要贴在UIView这块画布上的(CAMetalLayer 的 Framebuffer)    
