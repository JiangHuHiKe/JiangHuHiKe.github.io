...
<span style="color:red;font-weight:bold;">StatefulWidget + ChangeNotifier + InheritedWidget = 原生底层方案，不依赖任何三方库</span>
...
<span style="color:red;font-size:blod;">Provider:官方推荐三方库(非官方)。它是对ChangeNotifier + InheritedWidget的封装</span>     
...
<span style="font-weight:bold;">Dependency Graph 必须在 Widget 的构建阶段（build 或 didChangeDependencies）建立。</span>    
...
#### **四、其它**
MultiProvider 只是一个语法糖，最终还是会将多个Provider变成嵌套结构，它存在的目的就是提高代码的可读性。
...
Provider.value引用对象，不负责创建也不负责销毁
```text
...
## <a id="content4">Consumer 和 Selector</a>

<span style="color:red;font-size:bold;">Consumer 和 Selector 仍然属于 Provider 三方库</span>

#### **一、Consumer**      
<span style="color:red;">Consumer 不是减少 rebuild，而是改变 rebuild 的边界。</span>     
...
<span style="color:red;font-weight:bold;">在实际开发中，如果页面只依赖某个字段（如 name、count、themeMode），优先使用 Selector；<br>如果整个对象都会参与 UI 构建，使用 Consumer 即可。</span>
