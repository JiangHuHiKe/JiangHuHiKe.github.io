...
如果这是一个flutter application,在iOS中dart代码编译后就是App.framework          
...
如果这是一个flutter application,在iOS中<span style="font-weight:bold;color:red;">引擎层</span>代码编译后就是flutter.framework       
...
需要注意的是：Flutter 的嵌入层代码主要位于 Flutter 引擎的仓库中，具体分布在几个不同的目录中。     
shell/platform/android：包含 Android 平台相关的嵌入层实现代码。    
shell/platform/darwin/ios：包含 iOS 平台相关的嵌入层实现代码     


xy:dart代码 -> 引擎层 -> 嵌入层 -> 操作系统 -> 硬件，跟硬件打交道主要通过操作系统        
